...
echo "INSTANCE_NAMESPACE  EXTERNAL_IP     DUCKDNS_FQDN                     NOVNC_URL                         SUNSHINE_URL"
for ns in $(kubectl get ns -l app.kubernetes.io/part-of=steam-headless-fleet -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' 2>/dev/null); do
  [[ -n "$ns" ]] || continue
  dns_fqdn=$(kubectl get ns "$ns" -o jsonpath="{.metadata.annotations['steam-headless/duckdns-domain']}" 2>/dev/null || true)
  ...
  if [[ -z "$web_svc" ]]; then
    echo "$ns  <none>  ${dns_fqdn:-<none>}  <no web service>  <no web service>"
    continue
  fi
  ip=$(kubectl -n "$ns" get svc "$web_svc" -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || true)
  if [[ -z "$ip" ]]; then
    echo "$ns  <pending>  ${dns_fqdn:-<none>}  <pending>  <pending>"
  else
    echo "$ns  $ip  ${dns_fqdn:-<none>}  http://$ip:8083/  https://$ip:47990/"
  fi
done

...
delete_should_skip_home_backup() {
  local skip_flag last_home_backup_at
  skip_flag="$(metadata_get "$DELETE_SKIP_HOME_BACKUP_KEY")"
  last_home_backup_at="$(metadata_get "$HOME_BACKUP_AT_KEY")"
  [[ "$skip_flag" == "true" && -n "$last_home_backup_at" ]]
}
...
restore_games_from_archive() {
  ...
  if ! run_rclone_small copyto "$current_remote" "$GAMES_CURRENT_FILE"; then
    return 1
  fi
...
restore_games_legacy_sync() {
  ...
  rm -rf "$stage_dir"
  mkdir -p "$stage_dir"
  if ! run_rclone_large sync "$legacy_remote" "$stage_dir"; then
    return 1
  fi
...
backup_delete_state() {
  ...
  stop_stack
  if delete_should_skip_home_backup; then
    log "Skipping home backup during delete because a prior stop already persisted /home."
    set_metadata_values "$(jq -n --arg key "$DELETE_SKIP_HOME_BACKUP_KEY" '{($key): null}')"
  else
    set_metadata_values "$(jq -n --arg key "$DELETE_SKIP_HOME_BACKUP_KEY" '{($key): null}')"
    backup_home "$root" "$timestamp"
  fi
...
restore_create_state() {
  ...
  if restore_home "$root"; then
    ...
    set_restore_status "failed" "Home restore failed. Continuing with fresh home data."
  fi

  if restore_games "$root"; then
    if remote_file_exists "$(games_current_remote_path "$root")" || run_rclone_small lsf "$(legacy_games_dir_remote_path "$root")" >/dev/null 2>&1; then
      restored_any=1
    fi
  else
    set_restore_status "failed" "Games restore failed. Continuing with fresh games data."
  fi

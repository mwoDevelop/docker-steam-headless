...
apply_sunshine_state_credentials() {
  local pass
  local container_id
  pass="$(awk -F= '/^SUNSHINE_PASS=/{print substr($0,index($0,"=")+1)}' "$ENV" | tail -n1)"
  if [ -z "$pass" ]; then
    return 0
  fi

  container_id="$(sudo docker compose "${COMPOSE_ARGS[@]}" ps -q 2>/dev/null | head -n 1 || true)"
  if [ -n "$container_id" ] && sudo docker exec "$container_id" which sunshine >/dev/null 2>&1; then
    sudo docker exec "$container_id" sunshine --creds "admin" "$pass" >/dev/null 2>&1 || true
  fi
}
# Ensure running

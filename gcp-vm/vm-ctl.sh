...
ALLOW_SUN_PORTS=${ALLOW_SUN_PORTS:-tcp:47984,tcp:47989,tcp:47990,tcp:48010,tcp:27036-27037,udp:47998,udp:47999,udp:48000,udp:48002,udp:48010,udp:27031-27036}
...
deploy_instance() {
  local deploy_script="${ROOT_DIR}/gcp-vm/deploy-gce.sh"
...
ensure_remote_ready() {
  ...
  local setup_src="${ROOT_DIR}/gcp-vm/remote-setup.sh"
...
Examples:
  ./gcp-vm/vm-ctl.sh install prism
  ./gcp-vm/vm-ctl.sh install chrome
USAGE
}
...
case "$cmd" in
  up)
    ...
    # Wait up to ~60s for startup provisioning to finish writing env values.
    creds=""
    for i in $(seq 1 30); do
      creds=$(gcloud compute ssh "$GCE_NAME" --zone="$GCP_ZONE" --command "sudo awk -F= '/^SUNSHINE_USER=|^SUNSHINE_PASS=/{print \$0}' /opt/container-services/steam-headless/.env 2>/dev/null || true" 2>/dev/null || true)
    ...
    if [[ -n "$creds" ]]; then
      ...
      if [[ -n "$sun_pass" ]]; then
        echo "SUNSHINE_PASS=<hidden; stored in remote /opt/container-services/steam-headless/.env>"
      else
...
SUNSHINE_PASS=<hidden>"
      echo "(User copied to clipboard if supported; password remains only in remote .env.)"
    else
      echo "SUNSHINE_USER=admin"
      echo "SUNSHINE_PASS=<hidden; remote .env not readable yet>"
    fi
